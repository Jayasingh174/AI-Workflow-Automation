...
# Convert UPLOAD_DIR to a Path object safely
...
class BOQItem(BaseModel):
    item: str
    quantity: float = 0.0  # Safe default fallback
    unit: str
...
# ==========================================
async def safe_structured_llm_call(prompt: str, response_model: type[BaseModel]) -> Any:
    """
...
# ==========================================
async def document_agent(raw_text: str) -> Union[DocOutput, dict]:
    prompt = f"""
    ...
    """
    return await safe_structured_llm_call(prompt, DocOutput)


async def boq_agent(clean_text: str) -> Union[BOQOutput, dict]:
    prompt = f"""
    ...
    """
    return await safe_structured_llm_call(prompt, BOQOutput)


async def risk_agent(clean_text: str, boq_data: str) -> Union[RiskOutput, dict]:
    prompt = f"""
    ...
    BOQ Data: {boq_data}
    """
    return await safe_structured_llm_call(prompt, RiskOutput)


async def summary_agent(doc_data: str, boq_data: str, risk_data: str) -> Union[SummaryOutput, dict]:
    prompt = f"""
    ...
    Risk Data: {risk_data}
    """
    return await safe_structured_llm_call(prompt, SummaryOutput)
...
async def run_sequential_pipeline(file_path: str) -> dict:
    ...
    # 1. File Handling (Cleaned duplicate parsing block)
    ...
    if isinstance(boq, dict) and "error" in boq: 
        return boq
...
async def run_agent_loop(user_command: str) -> str:
    ...
    while True:
        # Note: passing raw dicts for 'tools' causes type errors with the client library.
        # We omit the explicit tools parameter here and let the model respond; tool calls
        # will still be handled via the tool_calls in the response object.
        response = await client.chat.completions.create(
            model=OPENAI_MODEL,
            ...
            assistant_msg["tool_calls"] = [
                {
                    ...
                    "type": t.type,
                    "function": {
                        "name": getattr(t, "name", None) or getattr(getattr(t, "function", None), "name", ""),
                        "arguments": getattr(t, "arguments", None) or getattr(getattr(t, "function", None), "arguments", "{}")
                    }
                }
        ...
        for tool_call in msg.tool_calls:
            name = getattr(tool_call, "name", None) or getattr(getattr(tool_call, "function", None), "name", "")
            try:
                arguments = getattr(tool_call, "arguments", None) or getattr(getattr(tool_call, "function", None), "arguments", "{}")
