...
from collections import defaultdict  # NOTE: still unused — see note at bottom
...
async def process_rfq(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # FIX #2 (part A): Populate "bom" from the LLM-extracted items.
        # process_rfq_bundle() has always read result.get("bom", []) expecting
        # each entry to have "item" and "quantity" keys — but this key never
        # existed before, so that branch always looped over an empty list.
        # We build it here from `extracted_items` (which use "name"/"qty")
        # and remap to the key names the bundle orchestrator actually expects.
        result["bom"] = [
            {
                "item": itm.get("name", "Unknown Item"),
        ...
        if file_path.lower().endswith((".dwg", ".dxf")):
            ...
            # FIX #2 (part B): Populate "cad_entities" from the parsed DXF blocks.
            # process_rfq_bundle() reads result.get("cad_entities", []) expecting
            # "item"/"qty" keys — this also never existed before, so DWG/DXF
            # files contributed zero entities to cross-document conflict
            # detection even after CAD parsing itself started working.
            #
            # We treat each unique block_name (from INSERT entities in the DXF —
            # i.e. repeated symbols/components placed in the drawing, like a
            # valve or fitting symbol) as a countable item, with quantity =
            # number of times that block appears in the drawing.
            blocks = cad_result.get("parsed_entities", {}).get("blocks", [])
            block_counts: Dict[str, int] = {}
            for b in blocks:
                name = b.get("block_name", "Unknown Block")
                block_counts[name] = block_counts.get(name, 0) + 1

            result["cad_entities"] = [
                {"item": name, "qty": qty} for name, qty in block_counts.items()
            ]
...
async def process_rfq_bundle(project_name: str, file_paths: List[str]) -> Dict[str, Any]:
    ...
    for i, raw_path in enumerate(file_paths):
        ...
        try:
            ...
            # NOTE (unchanged logic below): this code was always correct —
            # the bug was upstream in process_rfq() never populating the
            # keys being read here. Now that "cad_entities" and "bom" are
            # actually set (see FIX #2 in process_rfq above), this branch
            # will correctly pick them up.
